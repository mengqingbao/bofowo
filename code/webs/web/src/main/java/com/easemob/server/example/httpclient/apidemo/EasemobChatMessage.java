...
public class EasemobChatMessage {
    ...
    public static void main(String[] args) {
        ...
        JSONObject queryStrNode1 = new JSONObject();
        queryStrNode1.put("ql", "select * where timestamp>" + senvenDayAgo + " and timestamp<" + currentTimestamp);
        JSONObject messages1 = getChatMessages(queryStrNode1);
        ...
        // 第二页
        String cursor = messages2.get("cursor").toString();
	 ...
	 *
	 */
	public static JSONObject getChatMessages(JSONObject queryStrNode) {
		...
		try {
			...
			String rest = "";
			if (null != queryStrNode && queryStrNode.get("ql") != null && !StringUtils.isEmpty(queryStrNode.get("ql").toString())) {
				rest = "ql="+ java.net.URLEncoder.encode(queryStrNode.get("ql").toString(), "utf-8");
			}
			if (null != queryStrNode && queryStrNode.get("limit") != null && !StringUtils.isEmpty(queryStrNode.get("limit").toString())) {
				rest = rest + "&limit=" + queryStrNode.get("limit").toString();
			}
			if (null != queryStrNode && queryStrNode.get("cursor") != null && !StringUtils.isEmpty(queryStrNode.get("cursor").toString())) {
				rest = rest + "&cursor=" + queryStrNode.get("cursor").toString();
			}
