...
function initFreightScroll(){
    //计算高度
    $(".coupon-freefreight .coupon-enable").height(Math.ceil($(".coupon-freefreight .coupon-enable ul li").length/4) * 120);
...
//查询优惠券
function query_coupons_vertual() {
...
function refushVertualused(){
  ...
  if($("#hiddenCouponDiscount")[0]) {
    ...
    if($("#hiddenFreeFreight_coupon").val()>0){
      var couponShow;
      if(isHasVertualShow){
        couponShow="| 免运费券<em>1</em>张，抵用运费<em>"+parseFloat($("#hiddenFreeFreight_coupon").val()).toFixed(2)+"</em>元";
      }else{
        isHasVertualShow=true;
        couponShow="免运费券<em>1</em>张，运费<em>"+parseFloat($("#hiddenFreeFreight_coupon").val()).toFixed(2)+"</em>元";
      }
...
function addGiftCardVertual() {
  if ($("#lpkKeyPressFirst").val() == "" || $("#lpkKeyPressSecond" ).val() == ""
      || $("#lpkKeyPressThird").val() == "" || $("#lpkKeyPressForth").val() == "") {
