...
function consigneeList(selectId) {
  ...
  //yanwenqi 自提地址项目 判断当前地址是什么类型，是不是选中的隐藏在常用地址列表里的自提地址
...
//  $('.ui-scrollbar-main .consignee-scroll ul li').length-$('.selfPickInCommon').length;
  var _height = (($('.ui-scrollbar-main .consignee-scroll ul li').length-$('.selfPickInCommon').length) * 42)+"px";
  $('#consignee-addr .consignee-cont').css({'height': _height});
  $('.ui-scrollbar-wrap').css({'height':_height});
  //收货人地址滑动 start
  if(($('.ui-scrollbar-main .consignee-scroll ul li').length-$('.selfPickInCommon').length) > 4) {
	  if(consigneeScroll) {
		  consigneeScroll.resetUpdate($('.ui-scrollbar-main .consignee-scroll ul li').length-$('.selfPickInCommon').length);
	  } else {
...
//yanwenqi 自提地址项目  自提地址展开，如果当前选中的不是自提地址，选一条推荐自提给前面显示
...
//	  $(".consignee-item.item-selected").parent("li").css("display","list-item");
  ...
  //yanwenqi 自提地址项目 收起地址时判断要不要显示推荐,假如orderStore里有自提地址就删除推荐自提
...
//yanwenqi 自提地址项目 假如当前选中的是自提地址，且自提地址隐藏在常用地址的地址id与要切换的相同，也要刷新
  if(!isGiftBy) {
  ...
  jQuery.ajax({
    ...
    success: function(consigneeResult, textStatus) {
      ...
      if (consigneeResult.success) {
        ...
        //yanwenqi 自提地址项目 没有降级的情况下，下面的京东配送和上门自提只保留一个，隐藏另一个
    ...
    //yanwenqi 自提地址项目  此函数是常用地址间切换，切地址的时候，如果选中的不是自提地址，选一条推荐自提给前面显示
...
//      }
    //end
      //yanwenqi 自提地址项目 如果选择的是自提地址，不更新下面的这个
...
//yanwenqi 自提地址项目 编辑常用自提地址
function openEditSelfPickConsigneeDialog(id){
...
function doOutSku(outSkus, noStockType,address){
  ...
  if(noStockType == "600158"){
    ...
    //处理部分无货的情况
    try{
    	 var actionUrl="http://skunotify.jd.com/order/isSupportOrderBatchSku.action?skus="+outSkuArr;
    	 $.ajax({
    	      type : "get",
    	      dataType : "jsonp",
    	      async: false,  
    	      ...
    	      success : function(json) {
    	    	 ...
    	    	 if(resultflag==1){
    	          ...
    	          if(appendHtml){
    	        	log('gz_ord', 'gz_proc', 17, 'lzwhdxdtc');
...
function buildItemHtml(goodsObj){
  var goodsItem ="<div class='goods-item"  + (goodsObj.noStock ? " nostock-item" : "") + "'>"
        ...
        +      "<div class='p-name'> <a href='"+goodsObj.hrefUrl+"' target='_blank'>" + goodsObj.skuName + "</a><strong class='jd-price'>"+goodsObj.price+"</strong></div>"
...
function continueBuy(){
  ...
  if(isEmpty(skuIds)){
	  goOrder();
	    ...
	    var url = "http://skunotify.jd.com/storeOrderSubMvc/storeSubBatchSku.action";
        				...
        				if(ids[3] == dataResult.freightServiceVender[ids[2]].freightServiceSkuResultList[i].uuid){
        					$(".service-items div[name="+ids[0]+"]").each(function(){
        		        		...
        		        		if(dataResult.freightServiceVender[ids[2]].freightServiceSkuResultList[i].change){
        		        			$(this).children('.service-cont').children('.service-main').before('<span class="service-tips"><i class="s-ico"></i><i class="s-arr"></i>原服务不支持新收获地址，已重新匹配<b class="s-cls">x</b></span>')
